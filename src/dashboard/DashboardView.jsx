    ...
    <div className="group relative overflow-hidden rounded-2xl border border-white/5 bg-white/[0.03] p-5 backdrop-blur-sm transition-colors hover:border-white/10">
      ...
      {spark && (
        <div className="-mx-1 mt-2">
          <Sparkline data={spark} dataKey={sparkKey} color={color} />
        </div>
      )}
    </div>
  )
        ...
        <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-0.5 text-sm text-slate-400">
              ภาพรวมการเข้าใช้งานเว็บไซต์
              {data?.range && <span className="text-slate-500"> · {data.range.startDate} – {data.range.endDate}</span>}
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className="hidden items-center gap-2 rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5 sm:flex">
              <span className="grid h-6 w-6 place-items-center rounded-full bg-indigo-500/20 text-xs font-bold text-indigo-300">
            ...
            <button
              ...
              className="rounded-full border border-white/10 bg-white/[0.03] px-4 py-1.5 text-xs font-medium text-slate-200 transition-colors hover:bg-white/10"
            >
              ออกจากระบบ
            </button>
          </div>
                  ...
                  {devices.map((d, i) => (
                    <div key={i} className="flex items-center gap-2 text-xs">
                      ...
                      <span className="ml-auto font-medium text-slate-200">
                        {deviceTotal ? Math.round((d.users / deviceTotal) * 100) : 0}%
                      </span>
                    </div>
                  ))}
                </div>
