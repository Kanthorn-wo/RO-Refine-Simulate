...
- 1.11.10 (2026-08-19) — บั๊ก/security fix ชุดใหญ่ ไม่มี CHANGELOG (ทั้งหมดเป็น dashboard/internal/security): แก้ patch-notes modal บัง cookie bar ตอนเข้าเว็บครั้งแรก, i18n hardcode หลายจุด (HeroBanner/EventRateBanner/CookieConsent/DistChart/AvgCard/currency/aria-label), ปิดช่องโหว่ `monitor_daily_uptime`/`monitor_lighthouse_trend` view ข้าม RLS, เพิ่ม server-side validation กันข้อมูลเพี้ยนใน `api/refine.js`, gate `GET /api/stats?events=` ด้วย auth, รวม auth/BSB-range/today-formula/batch-cap ที่ซ้ำกันหลายไฟล์, ลบ dead code, บั๊ก dashboard อีกหลายจุด (realtime total count, settings cascade, error swallowing, MonitorView NaN, log ordering)
- 1.11.9 (2026-08-19) — ล็อกปุ่มตี manual ที่ +20 (เดิม Auto ล็อกอยู่แล้วแต่ manual หลุด — cap สูงสุดจริงของเกม) มี CHANGELOG ("ล็อกไม่ให้ตีบวกต่อเมื่อไอเทมถึง +20"); push เดียวกันยังรวม dashboard/security fix อีกหลายจุดที่ไม่ลง CHANGELOG (bot tagging, user activity modal, log ordering, monitor view RLS lockdown ฯลฯ)
- 1.11.8 (2026-06-20, `cac0f4b`) — dashboard: refine analytics v2 (ตาราง/RPC ใหม่) + แยก online presence settings ไม่มี CHANGELOG เพราะ dashboard-only
