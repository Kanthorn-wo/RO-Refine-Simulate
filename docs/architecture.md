...
- **Refine analytics ละเอียด** (dashboard เท่านั้น): `api/refine.js` (POST public เก็บ batch ผ่าน RPC `record_refine_batch`, GET owner-only อ่าน) + ตาราง `refine_log` (ring buffer 500), `refine_item_stats` (leaderboard), `refine_breakdown` (stone/bsb/result/level/item_type) — schema: `docs/sql/refine-analytics-schema.sql` (+ `docs/sql/refine-alltime-migration.sql`). Client เก็บผ่าน `recordRefineDetail()` ใน `handleRefine` (ทุก attempt: itemType/itemId(divine-pride)/level/stone/bsb/result). UI = `src/dashboard/RefineAnalytics.jsx` (อยู่ในหน้า Usage): สรุป + leaderboard (icon divine-pride) + breakdown charts + list รายครั้ง (badge แยกสี) + skeleton
- **ดูกิจกรรมรายผู้ใช้ (`src/dashboard/UserActivityModal.jsx`)**: คลิก vid ใน `ActivityFeed` หรือในรายการ log ของ `RefineAnalytics` → เปิด modal รวม log ของ vid นั้นจาก 2 แหล่ง — `usage_events` (GET `/api/stats?events=200`, filter client-side type visit/auto/simulate) + `refine_log` (GET `/api/refine?q=<vid>&limit=100`, owner-only ใช้ endpoint ค้นหาเดิม) เรียงเวลาล่าสุดก่อน — ไม่มี endpoint/ตารางใหม่ ใช้ของเดิมทั้งหมด จึงเห็นได้แค่เท่าที่ ring buffer ทั้งสองยังเก็บอยู่ (200/500 แถวล่าสุดรวมทุกคน)
