...
- **bannerRate** (banner "สำเร็จ X%") = rate ของระดับปัจจุบัน `getRate(..., Math.max(0, stack.length-1))` แยกจาก `currentRate` (rate รอบถัดไป ใช้ที่ปุ่ม "Rate:") — เมื่อ `mode === 'fail' && isItemLost` banner เปลี่ยนเป็นข้อความ "ไอเทมแตกสลาย" (สีดำ) แทน %
...
- `src/components/Toggle/index.jsx` — Toggle switch reusable
- `src/components/ReportButton/index.jsx` — ปุ่ม "แจ้งปัญหา" ลอย fixed มุมขวาล่าง (`z-40`, ต่ำกว่า stone modal `z-50`) เปิด Google Form ใน `_blank` + `rel="noopener noreferrer"`
