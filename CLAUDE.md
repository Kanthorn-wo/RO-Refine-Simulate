...
version ปัจจุบัน: **1.11.5** (CHANGELOG ล่าสุดอยู่ที่ 1.11.0 — 1.11.1–1.11.5 เป็นงาน internal/in-progress/polish/security ที่ผู้เล่นไม่เห็น จึงไม่มี entry ใหม่ ทำให้ `LATEST_CHANGELOG_VERSION` ≠ `APP_VERSION` โดยตั้งใจ; 1.11.2 = ธีม dark/light เฟส 1, 1.11.3 = cursor pointer, 1.11.4 = ธีม Phase 2–3,5, 1.11.5 = security (dashboard email allowlist + ซ่อน divine-pride key ผ่าน api/item proxy + npm audit)) (มีการ renumber CHANGELOG ทั้งชุดเมื่อ 2026-06-10 — เลขเดิม 1.9.3 กลายเป็น 1.6.0)
