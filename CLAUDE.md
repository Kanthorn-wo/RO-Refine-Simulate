...
- **ข้อมูล GA**: `api/ga.js` = Vercel Serverless Function เรียก GA4 Data API ด้วย service account (`@google-analytics/data`) — verify Supabase access token ก่อนทุก request (401 ถ้าไม่มี/ไม่ถูก) คืน totals + deltas (เทียบ 30 วันก่อนหน้า) + timeseries + topPages + devices + countries + events (FEATURE_EVENTS = custom event ของเว็บ)
...
- **UI**: `DashboardView.jsx` ใช้ recharts (KPI cards + sparkline + delta badge, area gradient, donut อุปกรณ์, bar events, leaderboard, skeleton loading)
...
version ปัจจุบัน: **1.9.0** (มีการ renumber CHANGELOG ทั้งชุดเมื่อ 2026-06-10 — เลขเดิม 1.9.3 กลายเป็น 1.6.0)
