...
### โมดูลย่อย
- `src/components/EventRateBanner/index.jsx` — แถบ "Event Rate Up" ลอยบนสุดเต็มจอ (**`fixed inset-x-0 top-0 z-40` — ห้ามใช้ sticky** เพราะ wrapper ใน App.jsx เป็น `flex justify-center` จะกลายเป็นคอลัมน์ซ้าย) render เฉพาะ `isEventRate = true` + มี spacer `h-8 sm:h-10` เป็น child แรกของ div max-w-4xl กันเนื้อหาโดนบัง สูง 56px mobile / 80px desktop พื้นหลังไฟลุก + ไอคอน RateUpIcon (เปลวไฟ gradient + ลูกศรขึ้นเด้ง) ด้วย CSS keyframes ใน `index.css` (`fire-shift`, `fire-flicker`, `ember-float`, `banner-drop`, `arrow-up-nudge`)
...
**คัดเฉพาะเรื่องสำคัญ:** อะไรที่ซ้ำซาก เล็กน้อยมาก หรือไม่มีประโยชน์ต่อ user (เช่น แก้สำนวนหลายรอบ, แก้บั๊กของฟีเจอร์ที่เพิ่งออกใน push เดียวกัน, งานภายใน) **ไม่ต้องใส่** หรือรวบเป็นข้อเดียวสั้น ๆ — patch notes สั้นแล้วคนอ่านมากกว่ายาว
