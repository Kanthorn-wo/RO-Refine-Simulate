...
- **bannerRate** (banner "สำเร็จ X%") = rate ของระดับปัจจุบัน `getRate(..., Math.max(0, stack.length-1))` แยกจาก `currentRate` (rate รอบถัดไป ใช้ที่ปุ่ม "Rate:")
...
`index.html` มี meta SEO ไทย (title/description/keywords), Open Graph, Twitter Card, JSON-LD, canonical, Google Search Console verification และ GA4 gtag (`G-52LVZ4FK1T`) — URL ทั้งหมดชี้ `https://ro-refine-simulate-i9a9.vercel.app` (deploy บน Vercel ที่ root path ไม่ต้องตั้ง `base`) มี `public/robots.txt` + `public/sitemap.xml` (Vite copy ไป dist root อัตโนมัติ) ยังค้าง `og-image.png` (meta อ้างถึงแต่ไฟล์ยังไม่มี)
...
- `src/constants/dailyData.js` ถูก overwrite โดย `scripts/update-daily-data.js` (รันเองด้วย `npm run update-data` เท่านั้น — workflow daily ที่เคยรันอัตโนมัติถูกลบแล้ว) ห้ามใส่ logic หรือ comment ที่ต้องคงไว้ในไฟล์นี้ — มันจะถูกเขียนทับ ถ้าจะเพิ่มฟิลด์ใหม่ต้องแก้ที่ template string ใน `update-daily-data.js` ด้วย
- `src/config-updates.js` เป็นไฟล์ append-only comment ที่ workflow บางตัวเขียนเข้าไป ไม่มีโค้ดทำงาน

### CI / deploy
Deploy หลักใช้ **Vercel** (auto build จาก push master, root path) — เว็บจริง `https://ro-refine-simulate-i9a9.vercel.app` workflow `auto-deploy.yml` / `daily-update-alt.yml` (เดิม update daily data + deploy gh-pages) **ถูกลบแล้ว** และ gh-pages/GitHub Pages ถูกตัดทิ้ง (เลี่ยง duplicate content) เหลือ workflow `random-contributions.yml` ตัวเดียว (cron ปั่น contribution graph — commit/push master ทุกวัน จึง trigger Vercel rebuild ด้วย)
