...
### โมดูลย่อย
- `src/components/EventRateBanner/index.jsx` — แถบ "Event Rate Up" ลอยบนสุดเต็มจอ (**`fixed inset-x-0 top-0 z-40` — ห้ามใช้ sticky** เพราะ wrapper ใน App.jsx เป็น `flex justify-center` จะกลายเป็นคอลัมน์ซ้าย) render เฉพาะ `isEventRate = true` + มี spacer `h-8 sm:h-10` เป็น child แรกของ div max-w-4xl กันเนื้อหาโดนบัง (spacer ยุบ/ขยายนุ่ม ๆ ด้วย transition + `-mb-5` หักล้าง gap ตอนยุบ) — **ย่อ/ขยายแบบ morph ชิ้นเดียว**: container เดิม `transition-all duration-500` เปลี่ยน ขนาด/margin/ความมน ระหว่างแถบเต็มจอ ↔ pill `w-[150px]` มุมขวาบน, เนื้อหา 2 ชั้น (เต็ม/pill) ซ้อน absolute แล้ว crossfade ด้วย opacity, ตอนเป็น pill ทั้งก้อนคือปุ่มขยายกลับ (`role="button"`) + เด้งต่อเนื่อง `.event-pill-mode` (ประกาศ fire-shift ซ้ำในลิสต์เพราะ animation shorthand ทับกัน, delay bob 0.5s รอ morph จบ) state `eventBarCollapsed` อยู่ใน Container สูง 56px mobile / 80px desktop พื้นหลังไฟลุก + ไอคอน RateUpIcon (เปลวไฟ gradient + ลูกศรขึ้นเด้ง) ด้วย CSS keyframes ใน `index.css` (`fire-shift`, `fire-flicker`, `ember-float`, `banner-drop`, `arrow-up-nudge`)
...
- **ENV ที่ต้องตั้งบน Vercel**: `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `GA_PROPERTY_ID`, `GA_CLIENT_EMAIL`, `GA_PRIVATE_KEY` (ค่าจริงอยู่ใน `.env.local` — gitignored; service account JSON เก็บใน `.secrets/` — gitignored)
- **Patch notes**: dashboard เป็น admin/analytics ภายใน **ห้ามใส่ใน CHANGELOG** (ผู้เล่นไม่เห็น)

### ระบบธีม dark/light (เฟส 1 — ยังทำไม่ครบ)
แนวทาง A = semantic tokens ผ่าน CSS variables (ไม่ใช้ `dark:` variant)
- **`src/index.css`**: `@theme inline` map token → utility (`bg-card/bg-sunken/text-body/text-dim/text-faint/border-line/border-line-soft` + accent `accent/accent-2/accent-3/accent-fg/accent-surface`) ค่าจริงอยู่ `:root` (dark default) และ `html.light` (light) สลับ runtime; **brand gradient นีออน** (เขียว→ฟ้า→ม่วง) = `--brand-gradient` + utility `.bg-brand` (ใช้เป็นแถบ/เส้น accent เท่านั้น ไม่ใช้เป็นพื้นปุ่มที่มีข้อความ เพราะตัวอักษรจม)
...
- **สถานะ**: แปลงเป็น token แล้วเฉพาะ `SimulatorPanel` (เฟส 1) — `Layout/index.jsx` (~325 จุด), modals, dashboard ยังเป็นสีฮาร์ดโค้ด dark-only (เฟส 2+) จึง light mode ยังครึ่ง ๆ; ปุ่ม toggle live แล้วบน production แต่ยังไม่ประกาศใน patch notes
