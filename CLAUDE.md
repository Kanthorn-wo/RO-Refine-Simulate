...
### โมดูลย่อย
- `src/components/EventRateBanner/index.jsx` — แถบ "Event Rate Up" sticky บนสุด (`sticky top-0 z-40`) render เฉพาะ `isEventRate = true` (อยู่นอก div max-w-4xl ใน Layout จึงเต็มจอ) สูง 56px mobile / 80px desktop พื้นหลังไฟลุกด้วย CSS keyframes ใน `index.css` (`fire-shift`, `fire-flicker`, `ember-float`, `banner-drop` + class `event-fire-*`, `event-ember`)
