...
- **`api/stats.js`** — ตัวนับการใช้งานรวม (social proof) เก็บใน Supabase: `usage_counters`/`usage_daily`/`usage_visitors`/`usage_events` (schema: `docs/usage-stats-schema.sql`). GET คืน totals + today (refine/stone/bsb/visits/new/returning/auto/simulate **Today**) + `daily[]` ตามช่วง + `events[]` (feed) + flag `showStats`/`showOnline`. POST รับ batch จาก `src/utils/usageStats.js` (sendBeacon). วันที่ใช้เวลาไทย (UTC+7) ทุกจุด
- **`usage_events`** มีคอลัมน์ `vid` (ผูก event กับผู้ใช้ anonymous) + `visitor_status` ('new'/'returning' เฉพาะ visit — เช็คก่อน `record_visit` สร้าง row)
...
- **คนออนไลน์ realtime** (`src/utils/useOnlineCount.js`): Supabase **Presence** (channel `online`, lazy-import supabase กัน main bundle บวม) — `track:true` ฝั่งผู้เล่น (การ์ดใน `UsageStats`), `track:false`/`enabled` ใน dashboard. Presence ไม่ต้องตั้งค่า DB เพิ่ม (เปิด default). flag `show_online` คุมการ์ดฝั่งผู้เล่น
...
- **Refine analytics ละเอียด** (dashboard เท่านั้น): `api/refine.js` (POST public เก็บ batch ผ่าน RPC `record_refine_batch`, GET owner-only อ่าน) + ตาราง `refine_log` (ring buffer 500), `refine_item_stats` (leaderboard), `refine_breakdown` (stone/bsb/result/level/item_type) — schema: `docs/refine-analytics-schema.sql`. Client เก็บผ่าน `recordRefineDetail()` ใน `handleRefine` (ทุก attempt: itemType/itemId(divine-pride)/level/stone/bsb/result). UI = `src/dashboard/RefineAnalytics.jsx` (อยู่ในหน้า Usage): สรุป + leaderboard (icon divine-pride) + breakdown charts + list รายครั้ง (badge แยกสี) + skeleton
